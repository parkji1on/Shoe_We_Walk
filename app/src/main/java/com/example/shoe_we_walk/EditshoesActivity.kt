...
class EditshoesActivity : AppCompatActivity() {
    val cntarray: Array<Int> = Array(11) { 0 }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for(item in Auth.itemList)
        {
            ...
            {
                if(item.item_id == 1)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_bee", "행복한꿀벌"))
                else if(item.item_id == 2)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_duck", "아기오리"))
                else if(item.item_id == 3)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_cat", "고양이"))
                else if(item.item_id == 4)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_clover", "네잎클로버"))
                else if(item.item_id == 5)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_flower", "보라색 꽃"))
                else if(item.item_id == 6)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_orange", "오렌지"))
                else if(item.item_id == 7)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_paeng", "펭귄"))
                else if(item.item_id == 8)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_bear", "분홍곰"))
                else if(item.item_id == 9)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_octopus", "문어"))
                else if(item.item_id == 10)
                    myAbleJibbitsAdapter.data.add(MyAbleJibbitsData(item.item_id, "jibbits_strawberry", "딸기"))
            }
    ...
    fun addUsed(myUsedJibbitsAdapter :MyUsedJibbitsAdapter,idx :Int, position :Int){
        if(idx == 1)
        {
